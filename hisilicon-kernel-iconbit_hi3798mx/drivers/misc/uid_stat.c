...
static void uid_work_func(struct work_struct *work)
{
	struct uid_stat *entry;
	struct uid_stat_work *uid_work;

	if (!work)
		return;

	uid_work = container_of(work, struct uid_stat_work, work);
	entry = uid_work->entry;
	if (entry)
...
static struct uid_stat *find_or_create_uid_stat(uid_t uid)
{
	...
	if (entry) {
		uid_work = kmalloc(sizeof(struct uid_stat_work), GFP_ATOMIC);
		if (!uid_work) {
			list_del(&entry->link);
			kfree(entry);
			spin_unlock_irqrestore(&uid_lock, flags);
		...
		uid_work->entry = entry;
		INIT_WORK(&uid_work->work, uid_work_func);
	}
	spin_unlock_irqrestore(&uid_lock, flags);
	if (uid_work)
		schedule_work(&uid_work->work);
