...
#define HI_AHCI_REG_DUMP(X) \
		printk("------------------[ Start ]--------------------\n"); \
		printk("Dump AHCI registers at %s %d\n", __func__, __LINE__); \
		hi_ahci_reg_dump(); \
		printk("------------------[  End  ]--------------------\n");


#define hi_sata_readl(addr) ({unsigned int reg = readl((unsigned int)addr); \
			printk("HI_AHCI(REG) %s:%d: readl(0x%08X) = 0x%08X\n", __func__, __LINE__, (unsigned int)addr, reg); \
			reg; })
		
#define hi_sata_writel(v, addr) do { writel(v, (unsigned int)addr); \
			printk("HI_AHCI(REG) %s:%d: writel(0x%08X) = 0x%08X\n", __func__, __LINE__, (unsigned int)addr, \
					(unsigned int)(v)); \
		} while (0)
