...
struct hinfc504_dbg_dump_item_t {
	unsigned short hour;
	unsigned short min;
	unsigned short sec;
	unsigned short msec;
...
struct hinfc504_dbg_dump_t {
	...
	struct hinfc504_dbg_dump_item_t logs[CONFIG_HINFC504_DBG_NAND_NUM_OF_LOGS];

	unsigned int read_index;
};

static DEFINE_MUTEX(dbg_dump_mutex);
static struct hinfc504_dbg_dump_t *dbg_dump = NULL;
			       ...
			       size_t count, loff_t *ppos)
{
	...
	struct hinfc504_dbg_dump_item_t *logs;
...
{
	struct hinfc504_dbg_dump_t *dump;

	if (dbg_dump)
		return 0;

	dump = vmalloc(sizeof(struct hinfc504_dbg_dump_t));
	if (!dump) {
		PR_ERR("Can't allocate memory.\n");
		return -ENOMEM;
	}
	memset(dump, 0, sizeof(struct hinfc504_dbg_dump_t));
...
static void dbg_dump_rw(struct hinfc_host *host, char op)
{
	...
	if (++dbg_dump->index >= CONFIG_HINFC504_DBG_NAND_NUM_OF_LOGS)
		dbg_dump->index = 0;
...
struct hinfc504_dbg_inf_t hinfc504_dbg_inf_dump = {
	"dump", 0,
	dbgfs_dump_init,
	dbgfs_dump_uninit,
	dbg_dump_read,
