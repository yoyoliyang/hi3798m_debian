...
struct hinfc504_dbg_erase_count_t {
...
static struct hinfc504_dbg_erase_count_t *dbg_erase_count = NULL;
	...
	while (pos < count) {
		...
		case 'c':
			...
			memset(dbg_erase_count->pe, 0,
			       dbg_erase_count->blocknum *
			       sizeof(struct hinfc504_dbg_erase_count_t));
...
static int dbgfs_erase_count_init(struct dentry *root, struct hinfc_host *host)
{
	...
	struct hinfc504_dbg_erase_count_t *erase_count;
...
struct hinfc504_dbg_inf_t hinfc504_dbg_inf_erase_count = {
	"erase_count", 0,
